...
class WebURLUtil {
  ...
  static  Map<String ,String> ACTIVITY_DETAIL_FIRST_DEPOSIT = {
    "url": "$_BASE_URL/promotion-detail/first-deposit?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}",
    "title": "Primeira recarga"
  };

  /// 活动-充值福利
  static Map<String ,String> ACTIVITY_DETAIL_DEPOSIT_BONUS = {
    "url": "$_BASE_URL/promotion-detail/deposit-bonus?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}",
    "title": "Recarga benefícios"
  };

  /// 活动-签到
  static Map<String ,String> ACTIVITY_DETAIL_CHECK_IN = {
    "url": "$_BASE_URL/promotion-detail/check-in?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}",
    "title": ""
  };

  /// 活动-宝箱
  static Map<String ,String> ACTIVITY_DETAIL_REWARD_BOX = {
    "url": "$_BASE_URL/promotion-detail/reward-box?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}",
    "title": "Jackpot de desempacotar"
  };

  /// 活动-邀请
  static Map<String ,String> ACTIVITY_DETAIL_INVITE = {
    "url": "$_BASE_URL/promotion-detail/invite?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}",
    "title": "Bónus de Convite"
  };

  /// 活动-每周累计投注
  static Map<String ,String> ACTIVITY_REFUND = {
    "url": "$_BASE_URL/promotion-detail/refund?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}",
    "title": "Apostas cumulativas semanais"
  };

  /// 活动Telegram
  static Map<String ,String> ACTIVITY_DETAIL_TELEGRAM = {
    "url": "$_BASE_URL/promotion-detail/telegram?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}",
  ...
  static Map<String ,String> REGISTER_RULES = {
    "url": "$_BASE_URL/privacy?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}",
    "title": "Privacy"
  };

  static Map<String ,String> getBannerTarPageParam(String p1){
    String url = "$_BASE_URL$p1?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}";
    Log.d("=======url:$url");
    return {
      "url": url,
      "title": "Promoção"
  ...
  static Map<String ,String> getGamePageParam(String url){
    String url = "url?is-app=1&t=${spUtil.getString(keyLoginToken) ?? ""}";
    Log.d("=======url:$url");
    return {
      "url": url,
      "title": "Promoção"
