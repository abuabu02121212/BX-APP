...
class MyState extends State<VerticalTabGroup> with TickerProviderStateMixin {
  ...
  Widget _buildItem(BuildContext context, int pos) {
    return Column(children: [widget.itemBuilder(context, pos, widget.controller.selectedIndexNotifier.value), SizedBox(height: widget.itemMargin)]);
  ...
  void autoScroll(int selectedPos) {
    ...
    double contentHeight = widget.size * widget.itemHeight + (widget.size) * widget.itemMargin;
